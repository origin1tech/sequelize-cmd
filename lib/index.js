...
function SqCmd(sequelize, options, logger) {
    ...
    defaults = {
        ...
        migration: {
            ...
            autoSeed: true,                // when true seeding will run after migrations.
            ...
            optionKeys: [                  // the keys to include when parsing models.
                'classMethods',
                'instanceMethods',
                'validate',
                'getterMethods',
    ...
    if(logger){
        ...
        log = function (msg, type){
            if(_.isFunction(logger[type]))
                logger[type](msg);
            else if(_.isFunction(logger.info))
                logger.info(msg);
            else
...
SqCmd.prototype.addModel = function addModel(name, properties, options, prefix, template) {
    ...
    if(modelNames.indexOf(name) !== -1 && (!flags.force && !flags.f))
        return this.exit('The model ' + name + ' already exists. Use --force or -f to overwrite.', 'warn');
 ...
 * @memberof SqCmd
 * @param {string|boolean} [name|auto] - the name of the migration or true if running autoMigrations.
...
SqCmd.prototype.addMigration = function addMigration(name, model, prefix, template) {
    ...
    var self = this,
        auto, targetPath, up, down;

    // listen for auto migrations
    // if first param is true
    auto = _.isBoolean(name) || flags.auto || flags.a || false;
    if(auto)
    ...
    this.getMeta().success(function (meta) {
        ...
        // if true generate migration for it.
        if(model && self.models[model]){
            ...
            if(meta && meta.data){
                previous = JSON.parse(meta.data.toString('utf8'));
                previous = previous[model.name] || undefined;
            }

            // compare to previous model for changes.
            compareResult = compare.call(self, model, previous);

            self.exit();

        } else {

            return log('Cannot generate migration using unknown model ' + model, 'warn');
        }
 ...
 * @memberof SqCmd
 */
SqCmd.prototype.addMigrations = function addMigrations() {
    return this.exit('auto migrations.')
};
 ...
 * @param {boolean} [strip] - when true strips rename property.
 * @returns {object}
 */
var load = SqCmd.prototype.load = function load(type, strip) {
    ...
    req = reqeach(p.join(cwd, typePath));

    // models import to sequelize instance
    // so we can access all model properties.
    if(type === 'models'){
        var simple = {};
        strip = strip ? ['rename'] : undefined;
       _.forEach(req, function (v,k){
        ...
        _.forEach(data, function (v,k) {
            data[k] = {
                ...
                attributes: helpers.normalizeAttributes(v.attributes, strip),
                associations: helpers.normalizeAssociations(v.associations),
                options: helpers.normalizeOptions(v.options, self.migConf.optionKeys)
            };
        });
 ...
 * @returns {promise}
 */
var getMeta = SqCmd.prototype.getMeta = function getMeta() {
 ...
 */
var exit = SqCmd.prototype.exit = function exit(msg, type){
    var code = 0;
