...
function SqCmd(sequelize, options, logger) {
    ...
    defaults = {
        ...
        migration: {
            ...
            modelNames: undefined           // model names converted to case (capitalize, lower, upper, camel, pascal, title, undefined).
        }
    };
    ...
    // merge the options.
    options.path = options.path || options.migrations;
...
SqCmd.prototype.addModel = function addModel(name, properties, options, prefix, template) {
    ...
    if(!name)
        return log('Generating a model requires a name.', 'warn');
...
SqCmd.prototype.addMigration = function addMigration(name, prefix, template) {
    ...
    if(!name)
        return log('Generating a migration requires a name.', 'warn');
