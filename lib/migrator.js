...
var fs = require('fs'),
    p = require('path'),
    moment, Utils, Migration, DataTypes;

module.exports = function migrator() {

    var ctx = this;
    moment = this.moment;
    Utils = require(p.join(this.sqPath, '/lib/utils'));
    DataTypes = this.DataTypes;
    Migration = require(p.join(this.sqPath, '/lib/migration'));
     ...
     * @constructor
     */
    var Migrator = function Migrator(sequelize, options) {
    ...
    /**
     * Runs migration by option method specified.
     * @memberof Migrator
     * @param {object} [options] - migrate options.
     * @returns {*}
     */
            ...
            self.getUndoneMigrations(function(err, migrations) {
                if (err) {
                    ...
                    if (migrations.length === 0) {
                        self.options.logging("There are no pending migrations.", 'warn')
                    } else {
    ...
    /**
     * Gets pending or undone migrations.
     * @memberof Migrator
    ...
    /**
     * Creates or finds migration metadata in database.
     * @memberof Migrator
    ...
    /**
     * Gets the last migration from the database.
     * @private
     * @memberof Migrator
     * @type {getLastMigrationFromDatabase}
     */
    ...
    /**
     * Gets the last migration from the database by specified id.
     * @private
     * @memberof Migrator
     * @type {getLastMigrationIdFromDatabase}
     */
    ...
    /**
     * Get a formated data string used for from/to in migration metadata.
     * @private
     * @memberof Migrator
     * @type {getFormattedDateString}
     */
    var getFormattedDateString = Migrator.prototype.getFormattedDateString = function(s) {
        ...
        try {
            ...
            return self.options.logging(s + ' is not a valid migration timestamp format! Use YYYYMMDDHHmmss!', 'error');
        }
    ...
    /**
     * Convert a string to date using moment and provided format.
     * @private
     * @memberof Migrator
     * @type {stringToDate}
     */
    ...
    /**
     * Called upon successful migration.
     * @private
     * @memberof Migrator
     * @type {saveSuccessfulMigration}
     */
    var saveSuccessfulMigration = Migrator.prototype.saveSuccessfulMigration = function(from, to, callback) {
        ...
        self.findOrCreateSequelizeMetaDAO().success(function(SequelizeMeta) {
            ...
            SequelizeMeta
                .create({ from: from.migrationId, to: to.migrationId, data: new Buffer(JSON.stringify(ctx.models), 'binary') })
                .success(callback);
        });
    };

    /**
     * Deletes undone migration
     * @private
     * @memberof Migrator
     * @type {deleteUndoneMigration}
     */
